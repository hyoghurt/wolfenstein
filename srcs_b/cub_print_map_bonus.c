...
/*   Updated: 2021/02/22 14:44:39 by hyoghurt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cub_print_ray(t_win *win, int scl)
{
	...
	while (win->start <= win->end && j <= win->width)
	{
		...
		while (win->map[(int)win->ray_y / scl][(int)win->ray_x / scl] != '1')
		{
			...
			if (win->ray_x < win->width && win->ray_y < win->height)
...
void	cub_print_map_wall(t_win *win, int scl_m)
{
	...
	while (win->map[++j])
	{
		...
		while (win->map[j][++i])
		{
			...
			{
				s1 = (j + 1) * scl_m;
				while (--s1 >= (j * scl_m) && s1 < win->height)
				{
					s2 = (i + 1) * scl_m;
					while (--s2 >= (i * scl_m) && s2 < win->width)
